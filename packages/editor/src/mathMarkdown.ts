/**
 * Markdown round-trip for math (display blocks and inline math).
 *
 * Same shape as the callout round-trip in calloutMarkdown.ts, and for the same
 * reason: BlockNote's Markdown converters know nothing about our custom `math`
 * block or `inlineMath` inline content, so we bracket their calls.
 *
 *  - EXPORT: swap every math node for a sentinel *before* BlockNote serializes,
 *    then substitute the LaTeX back in afterwards. Sentinels rather than raw
 *    `$…$` text because the serializer escapes backslashes, and a LaTeX body is
 *    mostly backslashes: `\frac` would come back as `\\frac`.
 *  - IMPORT: lift `$$ … $$` runs out of the raw Markdown into sentinel
